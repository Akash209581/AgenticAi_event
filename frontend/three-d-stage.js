...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    ...
    async _boot() {
      ...
      // Procedural studio HDRI: a soft sky/floor gradient with two big
      // soft-box highlights, prefiltered through PMREM so glossy and
      // metallic surfaces have something real to reflect.
      scene.environment = this._buildStudioEnvironment(THREE, renderer);
      ...
      this._loop = () => {
        if (this.onFrame) this.onFrame(performance.now() / 1000);
        controls.update();
    ...
    /** Builds a prefiltered environment map from a canvas-painted
     *  equirectangular studio (gradient + soft-box highlights). */
    _buildStudioEnvironment(THREE, renderer) {
      const W = 1024, H = 512;
      const cv = document.createElement('canvas');
      cv.width = W; cv.height = H;
      const ctx = cv.getContext('2d');
      const sky = ctx.createLinearGradient(0, 0, 0, H);
      sky.addColorStop(0.0, '#dfe6f2');
      sky.addColorStop(0.45, '#8e97a8');
      sky.addColorStop(0.5, '#4a5160');
      sky.addColorStop(1.0, '#14171d');
      ctx.fillStyle = sky;
      ctx.fillRect(0, 0, W, H);
      const softbox = (cx, cy, rx, ry, strength) => {
      ...
      softbox(W * 0.28, H * 0.22, 190, 130, 1.0);
      softbox(W * 0.74, H * 0.30, 150, 110, 0.75);
      softbox(W * 0.52, H * 0.06, 220, 90, 0.5);
